  ...
  return (
    <div className="min-h-screen px-4 pb-4 bg-app">
      ...
      <div className="relative pt-12 pb-6 overflow-hidden">
        ...
        <div className="absolute top-20 -left-10 w-40 h-40 rounded-full blur-3xl opacity-15"
          style={{ background: 'radial-gradient(circle, #ff6f91, #ff9671)' }} />
        <div className="absolute bottom-0 right-1/3 w-32 h-32 rounded-full blur-2xl opacity-10"
          style={{ background: '#ffc75f' }} />

        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          <p className="text-sm font-[Inter]" style={{ color: '#b0a8b9' }}>歡迎來到</p>
          <h1 className="text-4xl font-bold font-[Outfit] mt-1">
            <span className="text-gradient">🌴 峇里島</span>
          </h1>
          <p className="text-sm mt-1 font-[Inter]" style={{ color: '#b0a8b9' }}>
            Bali, Indonesia · {new Date().toLocaleDateString('zh-TW', { month: 'long', day: 'numeric', weekday: 'short' })}
          </p>
        </motion.div>

        {/* 漸層裝飾線 */}
        <div className="gradient-stripe h-[2px] w-24 mt-4 rounded-full opacity-80" />
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold font-[Outfit]" style={{ color: '#fefedf' }}>峇里島今日天氣</p>
              <p className="text-xs" style={{ color: '#b0a8b9' }}>熱帶島嶼氣候</p>
            </div>
          </div>
          <div className="flex gap-4">
            <div className="flex items-center gap-1 text-sm">
              <Thermometer size={14} style={{ color: '#ff9671' }} />
              <span style={{ color: '#d5cabd' }}>28-32°C</span>
            </div>
            <div className="flex items-center gap-1 text-sm">
              <Wind size={14} style={{ color: '#c493ff' }} />
              <span style={{ color: '#d5cabd' }}>微風</span>
            </div>
          </div>
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {[
          { label: '行程景點', value: totalAttractions, icon: '🗺️', color: '#c493ff' },
          { label: '今日行程', value: todayItinerary?.attractions.length ?? 0, icon: '📍', color: '#ff6f91' },
          { label: '旅遊天數', value: itinerary.length, icon: '📅', color: '#ffc75f' },
        ].map((stat, i) => (
          <motion.div
            key={stat.label}
          ...
          >
            ...
            <div className="text-xs mt-1 font-[Inter]" style={{ color: '#b0a8b9' }}>{stat.label}</div>
          </motion.div>
        ))}
      </div>
      ...
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-3">
          <h2 className="text-lg font-semibold font-[Outfit]" style={{ color: '#fefedf' }}>快速功能</h2>
            ...
            <Link to={action.to} key={action.to}>
              <motion.div
                ...
                style={{
                  ...
                  boxShadow: `0 4px 20px ${action.border}`,
                }}
              >
                {/* 右上角裝飾圓 */}
                <div className="absolute -top-4 -right-4 w-16 h-16 rounded-full"
                  style={{ background: 'rgba(254,254,223,0.1)' }} />
                <action.icon size={28} color="#fefedf" className="mb-3 relative z-10" />
                <h3 className="font-bold font-[Outfit] text-base relative z-10" style={{ color: '#fefedf' }}>
                  {action.label}
                </h3>
                <p className="text-xs mt-1 relative z-10" style={{ color: 'rgba(254,254,223,0.7)' }}>
                  {action.desc}
                </p>
              </motion.div>
            </Link>
      ...
      {todayItinerary && todayItinerary.attractions.length > 0 && (
        ...
        >
          <div className="flex items-center gap-2 mb-3">
            <h2 className="font-semibold font-[Outfit] text-lg" style={{ color: '#fefedf' }}>今日行程</h2>
            <div className="gradient-stripe h-[2px] flex-1 rounded-full opacity-40" />
          </div>
            ...
            {todayItinerary.attractions.slice(0, 3).map((att, i) => (
              <div key={att.id} className="glass-card rounded-xl p-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs truncate" style={{ color: '#b0a8b9' }}>{att.address || att.category}</p>
                </div>
              </div>
            ))}
            {todayItinerary.attractions.length > 3 && (
              <Link to="/explore">
                <p className="text-center text-sm py-2" style={{ color: '#c493ff' }}>
      ...
      <motion.div
        ...
        style={{
          ...
          border: '1px solid rgba(196, 147, 255, 0.25)',
        }}
      >
        {/* 彩色左側條 */}
        <div className="gradient-stripe absolute left-0 top-0 bottom-0 w-[3px] rounded-l-2xl" />
        <p className="font-semibold text-xs mb-1 pl-1" style={{ color: '#c493ff' }}>💡 AI 旅遊小提示</p>
        <p className="text-xs pl-1" style={{ color: '#d5cabd' }}>
